...
package com.example.sharan.ei_massignorder.data;
...
//    private static final UriMatcher sUriMatcher = buildUriMatcher();
    private OrderDbHelper mOpenHelper;
...
//addition
    //order.f_assigned = ?
    private static final String sAssignTypeSelection =
            OrderContract.OrderEntry.TABLE_NAME+
...
//    }
    ...
    public boolean onCreate() {
        mOpenHelper = new OrderDbHelper(getContext());
        return true;
    }
...
//addition
        long _id = db.insert(OrderContract.OrderEntry.TABLE_NAME, null, values);
                if ( _id > 0 )
                    returnUri = OrderContract.OrderEntry.buildOrderUri(_id);
                else
                    throw new android.database.SQLException("Failed to insert row into " + uri);
...
//        }
        ...
        try {
            for (ContentValues value : values) {
//                normalizeDate(value);
                long _id = db.insert(OrderContract.OrderEntry.TABLE_NAME, null, value);
                if (_id != -1) {
                    returnCount++;
                }
            }
